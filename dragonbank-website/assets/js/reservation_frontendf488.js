...
function check_fields() {
	...
	if (_captcha.length > 0) {
        ...
        $.ajax({
            ...
            success:function (response, textStatus, XMLHttpRequest) {
                if (!response) {
                    _captcha_input.css('border-color', 'red');
                    _captcha_resp = false;
                }
                else _captcha_input.css('border-color', '#e7dade');
            }
    ...
    _required_inputs.each(function () {
        ...
        if(jQuery(this).attr('type')=='checkbox'){
            if(!jQuery(this).is(':checked')){
                jQuery(this).next('label').css('border-color', 'red');
            } else {
                jQuery(this).next('label').css('border-color','#12A0A9');
            }
        } else {
            if (jQuery.trim(jQuery(this).val()) == '') {
                _return_val = false;
                ...
                jQuery(this).css('border-color', '#e7dade');
            }
