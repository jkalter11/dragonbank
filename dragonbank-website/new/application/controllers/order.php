	...
	public function validateForm()
	{
		...
		if ( $valid === TRUE && strcmp( strtoupper( $userCaptcha ), strtoupper( $word ) ) == 0 )
		{
			...
			if( sendNotification( "Customer Order", orderMsg($info), site_email() ) )
			{
				...
				flash_redirect('new/order', status('', 'Thank you for your interest in ordering Dragon Bank. We will be in touch shortly to confirm your order', 'green') );
			}
			else
			{
				// Sets flashdata and redirects safely.
				flash_redirect('new/order', status('', 'Email could not be sent. Please try again.', 'red') );
			}
		}
		else
		{
